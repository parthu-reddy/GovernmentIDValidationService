...
public class VerificationController {
    ...
    public ResponseEntity<?> verifyDrivingLicense(@Valid @RequestBody DLRequest request, Principal principal) {
        ...
        documentService.recordVerificationResult(
                executiveId, DocumentType.DRIVING_LICENSE, request.dlNumber(), request.documentUrl(), jsonResponse, expiry, status
        );
    ...
    public ResponseEntity<?> getVerificationSummary(@PathVariable UUID executiveId) {
        ...
        boolean allDocsApproved = java.util.Set.of(DocumentType.DRIVING_LICENSE, DocumentType.RC)
                .stream().allMatch(docType -> 
                    documents.stream().anyMatch(d -> d.getDocType() == docType && d.getApiVerificationStatus() == VerificationStatus.APPROVED)
                );
        ...
        var dlDocOpt = documents.stream()
                .filter(d -> d.getDocType() == DocumentType.DRIVING_LICENSE && d.getApiVerificationStatus() == VerificationStatus.APPROVED)
                .findFirst();
